...
leftColumn = [[sg.Text(text="Find Users Here")],
              [sg.Text(text='Search User By'),
               sg.InputOptionMenu(values=['UserID', 'Name', 'Address', 'Phone', 'CovidStatus'], key="option")],
...
layout2 = [[sg.Menu(menulayout)],
           [sg.Column(leftColumn), sg.VSeperator(color="lightblue", pad=(0, 0)),
...
while True:
    ...
    ## Updates info of the user according to the new data provided
        ...
        try:
            ...
            for i in range(len(cases)):
                plt.annotate(str(cases[i]), xy=(blocks[i], cases[i]), ha='center', va='bottom')
    ...
    ## Deletes saved passwords
        ...
        layout3=[[sg.Text("What can I help you with?"),
                  sg.InputOptionMenu(values=["Formats","Graphs","Inputting Data"],k="helpoption",)],
        ...
        while True:
            ...
            if event1==sg.WINDOW_CLOSED:
                ...
                if value1["helpoption"]=="Formats":
                    window3.FindElement("L@bel").Update('''The date has to be in YYYY-MM-DD format.Example:2020-09-12\n
                    The Address should start with Flat no/Plot no. Example:D-128 ''')

                elif value1["helpoption"]=="Graphs":
                    window3.FindElement("L@bel").Update("The Graph takes data from the Address and no. of cases(hence the block should be the first alphabet)"
                                                        "It can be edited using the options provided in the matplotlib library")
                elif value1["helpoption"]=="Inputting Data":
                    window3.FindElement("L@bel").Update('The code handles duplicate data for you so no need to worry about that.'
